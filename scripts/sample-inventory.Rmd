...
y <- readxl::read_xlsx(file.path(paths$box, 
                       ...
                       trim_ws = TRUE,
                       sheet = 2) %>%
  rename("Subject ID" = "Subject ID...1") %>%
  select(-"Visit 4") %>%
  full_join(x) %>% 
...
```{r}
makeLogical <- function(x) if_else(grepl("x", x, ignore.case = TRUE), 
                                   true = TRUE,
                                   false = FALSE,
                                   missing = FALSE)
...
long.dfll <- 
  dfll %>%
  select(id, visit, date, stool.vial, stool.tub, urine, blood) %>%
  gather(-id, -visit, -date, key = "biospecimen", value = "collected") %>%
  mutate(collected = as.logical(collected))
